...
def sentence1() -> Expr:
    ...
    firstSentence = A | B
    secondSentence = ~A % (~B | C)
    thirdSentence = disjoin([~A, ~B, C])
...
def sentence2() -> Expr:
    ...
    firstSentence = C % (B | D)
    secondSentence = A >> (~B & ~D)
    thirdSentence = ~(B & ~C) >> A
    forthSentence = ~D >> C

    return conjoin([firstSentence, secondSentence, thirdSentence, forthSentence])
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    if(findModel(premise & ~conclusion) == False):
        # Cannot find a model where premise is true and conclusion is false,
        # it means entail is true
        return True
    else:
        return False
    util.raiseNotDefined()
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    literal_pairs = combinations(literals, 2)
    for (first, second) in literal_pairs:
        res.append(logic.disjoin(~first, ~second))

    return logic.conjoin(res)    
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
